    ...
    const createCollegeUser = async (event) => {
        ...
        try {
            const response = await fetch("https://go-fingerprint.onrender.com/users/register", {
                method: "POST",
                credentials: "include",
                body: JSON.stringify({ username: newCollegeUsername, password: newCollegePassword }),
            });
...
<!-- Fixed Add Button -->
<button
    class="w-16 h-16 bg-black fixed bottom-12 right-8 text-white text-3xl font-medium rounded-full hover:text-white hover:bg-black duration-700 border shadow-xl flex items-center justify-center"
